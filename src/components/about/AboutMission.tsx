        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
          {cards.map((card, i) => (
            <Reveal key={card.title} y={30} delay={i * 0.12}>
              <motion.div
                className="group relative h-full overflow-hidden rounded-2xl border border-zinc-200 bg-white p-6 sm:p-7 lg:p-8 shadow-[0_8px_30px_rgba(0,102,204,0.06)] transition-all duration-300 hover:border-primary-300 hover:bg-primary-50/35 hover:shadow-[0_20px_50px_rgba(0,102,204,0.16)]"
              >
                ...
                <div className={`mb-5 inline-flex h-11 w-11 sm:h-12 sm:w-12 items-center justify-center rounded-xl bg-gradient-to-br ${card.gradient} text-white shadow-[0_4px_14px_rgba(0,102,204,0.22)] transition-transform duration-300 group-hover:scale-105`}>
                  {card.icon}
                </div>
                <h3 className="mb-3 text-lg sm:text-xl font-bold text-zinc-900">{card.title}</h3>
