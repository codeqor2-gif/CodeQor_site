        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {reasons.map((reason, i) => (
            <Reveal key={reason.title} y={30} delay={i * 0.08}>
              <motion.div
                className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white p-6 sm:p-7 lg:p-8 shadow-[0_8px_30px_rgba(0,102,204,0.06)] transition-all duration-300 hover:border-primary-300 hover:bg-primary-50/35 hover:shadow-[0_20px_50px_rgba(0,102,204,0.16)]"
              >
                <div className="mb-5 inline-flex h-11 w-11 sm:h-12 sm:w-12 items-center justify-center rounded-xl bg-gradient-to-br from-primary-600 to-primary-800 text-white shadow-[0_6px_16px_rgba(0,102,204,0.24)] transition-transform duration-300 group-hover:scale-105">
                  {reason.icon}
                </div>
                <div className="flex flex-1 flex-col">
                  <h3 className="mb-3 text-lg sm:text-xl font-bold text-zinc-900">{reason.title}</h3>
