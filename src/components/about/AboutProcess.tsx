...
import { FaCheck, FaRocket, FaCode, FaPalette, FaShieldAlt, FaLifeRing } from "react-icons/fa";
  ...
  return (
    <section ref={sectionRef} className="relative py-8 lg:py-12 overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-10">
        ...
        {/* Mobile / Tablet: Professional All Steps View */}
        <div className="lg:hidden mt-6">
          <div className="space-y-4">
            {steps.map((step, idx) => (
              ...
              >
                <div className="group relative overflow-hidden rounded-2xl border border-zinc-200/90 bg-white p-5 shadow-[0_4px_20px_rgba(0,102,204,0.06)] transition-all duration-300 hover:border-primary-300 hover:shadow-[0_8px_30px_rgba(0,102,204,0.12)]">
                  {/* Card Header: Node + Title + Duration */}
                  <div className="flex items-start gap-3.5">
                    {/* Step Icon Node */}
                    <div
                      className="shrink-0 flex h-12 w-12 items-center justify-center rounded-xl border-2 border-white text-white shadow-[0_4px_14px_rgba(0,102,204,0.18)]"
                      style={{ backgroundImage: `linear-gradient(135deg, ${step.hex}, ${step.hex}cc)` }}
                    >
                  ...
                  {/* Deliverable Names (without heading) */}
                  <div className="mt-3.5 flex flex-wrap gap-1.5 border-t border-zinc-100 pt-3">
                    {step.deliverables.map((d, dIdx) => (
                      <span
                        key={dIdx}
                        className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-100 bg-zinc-50/90 px-2.5 py-1 text-[11px] font-medium text-zinc-700"
                      >
                ...
                {/* Subtle vertical connector between step cards */}
                {idx < steps.length - 1 && (
                  <div className="flex justify-center my-2">
                    <div className="h-3.5 w-0.5 rounded-full bg-gradient-to-b from-primary-400 to-accent-300 opacity-50" />
                  </div>
                )}
              </motion.div>
            ))}
