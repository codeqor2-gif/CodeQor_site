...
function LogoCard({ name }: { name: string }) {
  return (
    <div className="mx-3 flex h-20 w-48 shrink-0 items-center justify-center rounded-xl border border-zinc-200 bg-white px-6 shadow-[0_6px_20px_rgba(0,102,204,0.05)] transition-all duration-300 hover:-translate-y-1 hover:border-primary-400/60 hover:shadow-[0_12px_32px_rgba(0,102,204,0.12)]">
  ...
  return (
    <section id="clients" className="relative overflow-hidden py-6">
      ...
      >
        ...
        >
          <div className={`flex w-max animate-marquee items-center ${paused ? "[animation-play-state:paused]" : ""}`}>
            {row.map((name, i) => (
        ...
        <div
          ...
          style={{ animationPlayState: paused ? "paused" : "running" }}
        >
          <div className={`flex w-max animate-marquee-reverse items-center ${paused ? "[animation-play-state:paused]" : ""}`}>
            {row.map((name, i) => (
