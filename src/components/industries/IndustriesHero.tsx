    ...
    <section className="relative flex min-h-screen items-center overflow-hidden bg-night-950 pt-24 pb-12 sm:pt-28 sm:pb-16 lg:min-h-[620px] lg:pt-32 lg:pb-20">
      ...
      <div className="relative mx-auto w-full max-w-7xl px-6 lg:px-10">
        <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-3xl">
          <div>
            <h1 className="text-4xl font-bold leading-[1.08] tracking-[-0.04em] text-white sm:text-5xl lg:text-[4rem]">Technology for <span className="bg-gradient-to-r from-sky-200 via-white to-sky-300 bg-clip-text text-transparent">every industry.</span></h1>
          </div>
          <p className="mt-6 max-w-2xl text-base leading-7 text-blue-100/75 sm:text-base">We combine deep industry understanding with modern technology to solve real business challenges and create lasting advantage.</p>
        </motion.div>
      </div>
    </section>
  );
